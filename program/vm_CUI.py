...
def executedcommand(stack,rstack,lstack,com,opr,back_com,back_opr,pc,pre,top,rtop,ltop,address,value,tablecount,variable_region,lock,process_number,process_path,count_pc,process_count,terminate_flag,flag_number,mlock,mlock2,program_counter,q,q2,q3,mode,mchange_flag,monitor_process_count,now_process_count,process_back_ori_num,step_flag):
    if com[pc]==1 and (mode.value==0 or mode.value==3 or mode.value==4):#push　演算スタックに即値を積む
        top=push(opr[pc],stack,top)
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==2 and (mode.value==0 or mode.value==3 or mode.value==4):#load　共有変数スタックから値を読み出し演算スタックに積む
        value.acquire()
        ...
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==3 and (mode.value==0 or mode.value==3 or mode.value==4):#store 変数の値を更新し値スタックにそれまでの変数の値及びプロセス番号とパスを積む
        value.acquire()
        ...
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==4 and (mode.value==0 or mode.value==3 or mode.value==4):#jpc スタックトップの値をポップしその値が1なら被演算子の番地にジャンプする
        (c,top)=pop1(stack,top)
        if c==1:
            pre=pc
            pc=opr[pc]-2
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==5 and (mode.value==0 or mode.value==3 or mode.value==4):#jmp　無条件で被演算子の番地にジャンプする
        pre=pc
        pc=opr[pc]-2
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==6 and (mode.value==0 or mode.value==3 or mode.value==4):#op　被演算子の種類の演算を行う
        if (opr[pc])==0:#'+'
            (c,top)=pop1(stack,top)
        ...
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==7 and (mode.value==0 or mode.value==3 or mode.value==4):#label　ラベルスタックにジャンプ前のPCの値とプロセス番号を積む
        ...
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif back_com[pc]==21 and mode.value==1:#rjmp　ラベルスタックから値を取り出しそのPCにジャンプする
    ...
    elif back_com[pc]==22 and mode.value==1:#restore　値スタックから値を取り出し共有変数スタックに保存する
        ...
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif (com[pc]==8  and (mode.value==0 or mode.value==3 or mode.value==4)) or (back_com[pc]==23 and mode.value==1):#par　並列ブロックの開始と終了を示す
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==9 and (mode.value==0 or mode.value==3 or mode.value==4):#alloc　新しい変数の番地を確保し初期値を0にする
        if args[2]=='f' or args[2]=='df':
            with open("variable_table.txt",'r') as f:
                t1=f.read().split('\n')
            var_flag=0
        ...
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==10 and (mode.value==0 or mode.value==3 or mode.value==4):#free　変数の番地の解放を示し値スタックにそれまでの値を保存する
        ...
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==11 and (mode.value==0 or mode.value==3 or mode.value==4):#proc　手続きの開始　label命令とblock命令を行う　返り番地を演算スタックに積む
        ...
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==12 and (mode.value==0 or mode.value==3 or mode.value==4):#ret　手続きの終了　演算スタックから返り番地を取り出しその番地にジャンプする
        p_lstack=[]
    ...
    elif com[pc]==13 and (mode.value==0 or mode.value==3 or mode.value==4):#block　パスを追加
        ...
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==14 and (mode.value==0 or mode.value==3 or mode.value==4):#end　パスの削除
        for i in range(0,len(process_path),1):
            if process_path[i] == '.':
                process_path=process_path[i+1:len(process_path)]
                break
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==15 and (mode.value==0 or mode.value==3 or mode.value==4):#fork 並列プロセスの生成
        ...
        lock.acquire()#lock.acquire()
        return (a,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==16 and (mode.value==0 or mode.value==3 or mode.value==4):#merge 並列ブロックの終了を示す
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==17 and (mode.value==0 or mode.value==3 or mode.value==4):#func 関数の開始 label命令とblock命令を行い演算スタックトップの一つ下に帰り番地を積む
        ...
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif com[pc]==18 and (mode.value==0 or mode.value==3 or mode.value==4):#f_return 関数の終了 演算スタックの一つしたの値を取り出しその番地にジャンプする
        p_lstack=[]
        with open("label_stack.txt",'r') as f:
            p_lstack=f.read().split()
        f.close()
        ...
        pre=pc
        return (c,pre,stack,top,rtop,tablecount,process_path)
    elif back_com[pc]==24 and mode.value==1:#r_alloc 逆向きのalloc命令 値スタックから値を取り出し 確保した共有変数スタックの番地に保存する
        s2=re.search(r'([a-z]\d+\.)+',rstack[rtop.value+1])
        process_path=s2.group()+'E'
        with open("variable_table.txt",'r') as f:
            t1=f.read().split('\n')
        ...
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif back_com[pc]==25 and mode.value==1:#r_free 逆向きのfree命令 変数の解放を示す
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
    elif back_com[pc]==26 and mode.value==1:#r_fork 逆向きのfork命令 前向きとは逆順に並列テーブルを参照する
        lock.release()
        process={}
        start_process_count = process_count.value
        now_process_count.value=now_process_count.value-1
        ...
        lock.acquire()
        return (a,pre,stack,top,rtop,tablecount,process_path)
    elif back_com[pc]==27 and mode.value==1:#r_merge 逆向きのmerge
        pre=pc
        return (pc+1,pre,stack,top,rtop,tablecount,process_path)
...
def monitor_exec_command(stack,top,com,opr,process_path,living_flag):
    if com==1:#push　演算スタックに即値を積む
        top=push(opr,stack,top)
        return (stack,top)
    elif com==2:#load　共有変数スタックから値を読み出し演算スタックに積む
        c=value[search_table(opr,process_path)]
        top=push(c,stack,top)
        return (stack,top)
    elif com==6:#op　被演算子の種類の演算を行う
        if (opr)==0:#'+'
            (c,top)=pop1(stack,top)
            (d,top)=pop1(stack,top)
            top=push(c+d,stack,top)
        ...
        if living_flag==1:
            top=push(1,stack,top)
            #print("living_flag "+str(living_flag))
        elif living_flag==0:
            top=push(0,stack,top)
        return (stack,top)
    elif com==31:#living
...
if __name__ == '__main__':
    ...
    if args[2]=='f' or args[2]=='df':
        ...
        with open("record_contract_table.txt",'w') as f:
            f.write("")
    elif args[2]=='b' or args[2]=='db':#逆方向も同様に各ファイルを開き,ラベルスタック,値スタックの情報をリストに保存する
        with open("variable_table.txt",'w') as f:
            f.write("")
        f.close()
        with open("label_stack.txt",'r') as f:
    ...
    elif args[2]=='f':#順方向バイトコードから逆方向バイトコードへ変換
        process = Process(target=execution,args=(com,opr,back_com,back_opr,0,count_pc,count_pc,stack,address,value,tablecount,rstack,lstack,rtop,ltop,endflag0,variable_region,lock,process_number,process_path,process_count,terminate_flag,0,mlock,mlock2,program_counter,q,q2,q3,mode,mchange_flag,monitor_process_count,now_process_count,process_back_ori_num,step_flag))
        process.start()
        process.join()
    elif args[2]=='b':#逆方向実行
